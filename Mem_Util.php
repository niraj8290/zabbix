<?php
    ...
    $zabbix_url = 'http://35.154.191.205/zabbix/api_jsonrpc.php';
    $auth_token = '810b18a242c9ffae9071774770085c61a486b31ad761fc0b92f9ef1afc88f7b2';  // Replace with your actual Auth token
...
function get_host_ids($group_id) {
    ...
    function get_cpu_cores($hostid) {
        // Call to Zabbix API for CPU core or memory data
        $cpu_cores_response = zabbix_api_call('item.get', [
            'output' => ['lastvalue'],
            'hostids' => $hostid,
            'search' => ['key_' => 'vm.memory.size[total]'], // Memory in bytes
        ]);
    
        // Debugging - Log the API response
        error_log('CPU cores response: ' . print_r($cpu_cores_response, true));
    
        // Check if the response contains valid data for memory
        if (!empty($cpu_cores_response['result']) && isset($cpu_cores_response['result'][0]['lastvalue'])) {
            // Convert memory from bytes to GB
            $memory_in_gb = round((int)$cpu_cores_response['result'][0]['lastvalue'] / (1024 ** 3), 2);
            return $memory_in_gb . ' GB'; // Return memory in GB format
        }
        ...
        // If WMI data is found, return the number of logical processors
        if (!empty($wmi_response['result']) && isset($wmi_response['result'][0]['lastvalue'])) {
            return (int)$wmi_response['result'][0]['lastvalue'];
        }
    
        // If no valid data is found, return "N/A"
        return 'N/A';
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $start_date && $end_date && !empty($selected_groups)) {
    try {
        ...
        function exportToExcel($summary_data) {
            ...
            $filename = 'memory_utilization_report.xlsx';
        ...
        function exportToCSV($summary_data) {
            ...
            header('Content-Disposition: attachment; filename="memory_utilization_report.csv"');
...
<!DOCTYPE html>
    ...
    <head>
        ...
        <title>Zabbix Memory Utilization Report</title>
...
<div class="widget-container">
        ...
        <div class="parameter-form">
            ...
            <h1> Memory Utilization Report</h1>
...
<form id="report-form" method="POST" action="">
    ...
    <button type="submit" name="generate_report" value="1">Generate Report</button>
        ...
        <div class="report-form2">
            ...
            <div class="printable">
                <h2>Memory Utilization Report</h2>
